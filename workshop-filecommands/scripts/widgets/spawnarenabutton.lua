...
local function OnClick()
    SendModRPCToServer(MOD_RPC.filecommands.DoSafe, "spawnArena()")
end

local SpawnArenaButton = Class(ImageButton, function(self)
--    TextButton._ctor(self, "images/ui.xml", "blank.tex")--"images/twitch_button.xml", "twitch_button.tex")
  ...
  --self.image:SetTint(.5, 1, .5, 1)
    ...
    self:SetTooltip("Spawn arena")
    self:SetText("A")
end)

function SpawnArenaButton:SnapToPosition(x, y, z)
    self:StopUpdating()
...
function SpawnArenaButton:TweenToPosition(x, y, z)
    self.x = x
    self.y = y
    self.z = z
    self:StartUpdating()
...
function SpawnArenaButton:OnUpdate(dt)
