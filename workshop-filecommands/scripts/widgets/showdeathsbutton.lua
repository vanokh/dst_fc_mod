...
local function OnClick()
    SendModRPCToServer(MOD_RPC.filecommands.UpdateDeaths, ThePlayer)
end

local ShowDeathsButton = Class(ImageButton, function(self)
--    TextButton._ctor(self, "images/ui.xml", "blank.tex")--"images/twitch_button.xml", "twitch_button.tex")
  ...
  --self.image:SetTint(.5, 1, .5, 1)
    ...
    self:SetTooltip("Show deaths")
    self:SetText("D")
end)

function ShowDeathsButton:SnapToPosition(x, y, z)
    self:StopUpdating()
...
function ShowDeathsButton:TweenToPosition(x, y, z)
    self.x = x
    self.y = y
    self.z = z
    self:StartUpdating()
...
function ShowDeathsButton:OnUpdate(dt)
